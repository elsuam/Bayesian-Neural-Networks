...
Artificial neural networks composed of one or more layers in between the input data and the output results.  In is common for a N-layered network to be recognized as such by the number of N layers that have tunable weights.  As shown in the figure to the right, a network that has an input layer, one hidden layer, and an output has two layers of tunable weights (\emph{input} and \emph{hidden}) is a two-layer network.  A neural network is based on a feed-forward mapping, in which connections exist only directly between any two subsequent layers, and each unit (or *neuron*) is connected to all neurons in its adjacent layer(s).  Such feed-forward designs are non-linear mappings between a set of input variables and a set of output variables, in which direct connections between layers are transformed by non-linear activation functions.\cite{bishop2006pattern}.
